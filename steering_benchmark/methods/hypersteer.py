...
from steering_benchmark.training.hypersteer import train_hypersteer


@register_method("hypersteer")
class HyperSteerSteering(SteeringMethod):
    def __init__(self, config: Dict) -> None:
        super().__init__(config)
        self._cost = {}

    def fit(self, model, dataset, run_cfg: Dict):
        hyper_cfg = self.config.get("hypersteer", {})
        result = train_hypersteer(model, hyper_cfg)
        ...
        layer = int(self.config.get("layer", 0))
